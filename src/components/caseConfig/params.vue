...
<!-- 分步骤执行结果弹窗 -->
    ...
    <el-dialog
      ...
      @close="closeChange()"
    >
    <span style="color:red;font-size:15px;">拖动方块调整顺序</span>
      <!-- 调整列表用例表单 -->
      <div style="background: rgb(183, 184, 177);">
        ...
        <draggable v-model="groupCaseList" @update='update' class="drag">
            <transition-group style="margin-top:5px">
                <div v-for="item in groupCaseList" :key="item.id">
                    <!-- <el-button type="info">{{item.name}}</el-button> -->
                    <span style="margin-left:15px;">{{item.name}}</span>
                 <el-button size="mini" type="danger" style="float:right;margin-right:10px" icon="el-icon-delete" circle></el-button>
                </div>
            </transition-group>
        </draggable>
      </div>
...
export default {
  ...
  methods: {
    update(){
        console.log(this.groupCaseList);
    },
    openDebug () {
      console.log('打开')
    },
...
.drag div {
  ...
  border: 2px solid rgb(244, 247, 111);
  background: white;
